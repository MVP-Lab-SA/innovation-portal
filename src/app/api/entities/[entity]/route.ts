...
export async function GET(request: NextRequest, { params }: { params: { entity: string } }) {
  ...
  if (!config) return NextResponse.json({ error: `Unknown entity: ${params.entity}` }, { status: 404 });
  const handler = createListHandler(config.model, config.options);
...
export async function POST(request: NextRequest, { params }: { params: { entity: string } }) {
  const config = MODEL_REGISTRY[params.entity];
  if (!config) return NextResponse.json({ error: `Unknown entity: ${params.entity}` }, { status: 404 });
  const handler = createCreateHandler(config.model, config.options);
  return handler(request);
}
